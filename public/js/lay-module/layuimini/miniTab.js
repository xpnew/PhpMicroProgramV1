...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
        ...
        /**
         * 刷新当前tab方法
         * 不指定tabid刷新	miniTab.reloadCurrentByIframe();
	     * 指定tabid刷新	miniTab.reloadCurrentByIframe(tabId);
         */
        reloadCurrentByIframe: function () {
            let iframe = window.top.layui.$(".layui-tab-item.layui-show").find('iframe');
            if (arguments.length !== 0) {
                if (!$(".layui-tab-item.layui-show").has("id")) {
                    iframe = window.top.layui.$('.layui-tab-content div[lay-id="' + arguments[0] + '"]').find('iframe');
                }
            }
            if (iframe.length !== 0) {
            ...
            element.on('tab(' + options.filter + ')', function (data) {
                ...
                if ($('.layuimini-menu-left').attr('layuimini-tab-tag') === 'add') {
                    ...
                    if (options.clickHomeTabRefresh) {
                        miniTab.reloadCurrentByIframe(tabId);
                    }
                }
