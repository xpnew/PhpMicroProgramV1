    ...
    var miniAdmin = {
        ...
        render: function (options) {
            ...
            if (options.iniUrl == null && options.data == null) {
                var tips = "请先将项目部署至web容器（Apache/Tomcat/Nginx/IIS/等）或者往render函数中data字段设置数据，否则部分数据将无法显示";
            ...
            if (/http(s*):\/\//.test(location.href)) {
                $.getJSON(options.iniUrl, function (data) {
                    if (data == null) {
                        ...
                        let ErrorCode = data.ErrorCode ||  data.errorCode || data.code  || 0 ;
                        if(0 < ErrorCode){
                            console.log('加载菜单 出错： ', data);
                            let msg = data.msg || data.Msg || data.ErrorMessage || '初始化失败';
                            miniAdmin.error(msg,999999);
                            //miniAdmin.deleteLoader(options.loadingTime);

                            return false;
                        }
                        let OkCode = data.code || data.Code || data.type || data.Type || 0 ;
                        if( 0 !== OkCode ){
                            console.log('加载菜单 出错： ', data);
                            let msg = data.msg || data.Msg || data.ErrorMessage  || data.Content || '初始化失败';
                            miniAdmin.error(msg,999999);
                            //miniAdmin.deleteLoader(options.loadingTime);
                            return false;
                        }
         ...
         * @returns {*}
         */
        error: function (title,waittimer = 3000) {
            // console.log('错误信息：',title);
            // console.log('waittimer',waittimer);
            // layer.alert(title);
            // return;
            return layer.msg(title, { icon: 2, shade: this.shade, scrollbar: false, time: waittimer, shadeClose: true });
        },
